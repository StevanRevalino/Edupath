...
export default function ModalResetPassword({ isOpen, onClose }: Props) {
  ...
  const handleChangeOtp = (value: string, index: number) => {
    ...
    if (newOtp.every((digit) => digit !== "")) {
      ...
      if (userInputOtp === serverOtp) {
        setIsVerified(true);
        alert("OTP sesuai! Silakan ganti password.");
      } else {
        alert("OTP salah!");
      }
    }
  ...
  const handleSubmit = async () => {
    if (!isVerified) return alert("Verifikasi email terlebih dahulu");
    ...
    if (!res.ok) return alert(data.message || "Gagal reset password");
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center scale-125">
      <div className="bg-white p-6 rounded-xl w-[600px] shadow-xl relative">
        ...
        <div className="grid grid-cols-2 gap-4 mb-3">
          ...
          <div>
            <label className="text-sm block mb-1 text-lef">
              Konfirmasi Password
            </label>
            <input
              type="password"

        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full">
          ...
          {!isVerified && onVerify ? (
            <button
              type="button"
              onClick={onVerify}
              className="bg-[#6CCBFF] hover:bg-[#4BB8FF] text-white px-4 sm:px-3 py-2.5 sm:py-2 rounded-full text-sm sm:text-lg font-semibold cursor-pointer whitespace-nowrap flex-shrink-0"
            >
              {verifyButtonText}
            </button>
          ) : isVerified ? (

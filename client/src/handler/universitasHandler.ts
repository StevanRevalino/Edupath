...
class UniversitasService {
  ...
  async searchUniversitasByName(
    name: string,
    signal?: AbortSignal
  ): Promise<UniversitasResponse> {
    try {
      const token = TokenManager.getToken();

      const response = await axios.get(`${API_URL}/api/universitas/search`, {
        params: { nama: name.trim() },
        signal,
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  async getUniversitasWithFilters(params: {
    ...
    try {
      const token = TokenManager.getToken();
      const { searchKeyword, provinsi, akreditasi, limit } = params;
      ...
      if (searchKeyword && searchKeyword.trim().length > 0) {
        ...
        // Get universitas with filters
        url = `${API_URL}/api/universitas`;

        if (provinsi && provinsi !== "Semua") {
          queryParams.provinsi = provinsi;
        }
